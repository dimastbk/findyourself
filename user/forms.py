...
class RegistrationForm(CssClassFormMixin, forms.ModelForm):
    ...
    password1 = forms.CharField(
        label='Пароль', max_length=30, min_length=8, widget=PasswordInput(),
    )
    password2 = forms.CharField(
        label='Подтверждение пароля', max_length=30, min_length=8, widget=PasswordInput(),
    )

    password_mismatch = 'Пароли не совпадают.'  # noqa: S105 # nosec
    ...
    def clean_password2(self):
        ...
        if password1 and password2 and password1 != password2:
            raise forms.ValidationError(
                self.password_mismatch, code='password_mismatch',
            )
        return password2
...
class UserForm(CssClassFormMixin, forms.ModelForm):
    def clean_username(self):
        ...
        user = User.objects.filter(username=username).exclude(pk=user_id).exists()
        if user:
